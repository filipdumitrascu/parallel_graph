...
typedef struct os_threadpool {
	...
	/**
	 * Mutex for protecting access to the thread pool data,
	 * a condition variable for signaling when tasks are added to the queue
	 * and an atomic variable to check if no task is available
	 */
    pthread_mutex_t mutex;
    pthread_cond_t cond;
	atomic_int tasks_added;
} os_threadpool_t;
